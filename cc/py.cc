...
PYBIND11_MODULE(acmacs_draw_backend, m)
{
    ...
    py::class_<acmacs::surface::Surface>(m, "Surface")
            ...
            .def("border", [](Surface& aSurface, std::string color, double width) { aSurface.border(Color(color), Pixels{width}); }, py::arg("color"), py::arg("width"))
            .def("background", [](Surface& aSurface, std::string color) { aSurface.background(Color(color)); }, py::arg("color"))
